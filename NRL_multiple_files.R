...
server <- function(input, output) {
  ...
  observeEvent(eventExpr = input$png, handlerExpr = { 
    ...
    write.table(as.data.frame(rv$m[-1,]),paste0(gsub('NRL_aggregate_','',gsub('_CTCF.txt','',filed[value(),])),'_vals.txt'),row.names = F,col.names = F, sep='\t')
  ...
  output$plot1 <- renderPlot({
    ...
    x=ceemdan(y, ensemble_size = 1000)
